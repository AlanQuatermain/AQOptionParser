...
//  AQOptionRequirementGroup.m AQOptionParser  Created by Jim Dovey on 2012-05-23.  Copyright (c) 2012 Jim Dovey. All rights reserved.
...
- (id) initWithOptions:(NSArray*) options type: (AQOptionRequirementType) type
{
  ...
  for (AQOption * option in _options) [option addedToRequirementGroup: self];
  return self;
}

- (BOOL) containsOption:(AQOption*)option { return [_options containsObject: option]; }

- (BOOL) isMatched { return !!_firstMatched; }
...
- (void) optionWasMatched: (AQOption*) option
{
  if ( _type == AQOptionRequirementExclusive && _firstMatched != option )
    ...
    return [NSException raise: AQOptionExclusiveRequirementMultipleMatchException format: @"Matched mutually exclusive options %@ and %@", _firstMatched, option];
